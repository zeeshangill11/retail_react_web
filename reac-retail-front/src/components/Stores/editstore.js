...
class editstore extends Component {
  ...
  async componentDidMount (){
    ...
    if(main_data[0].storeid !== null){ this.setState({id : main_data[0].storeid}) }
    if(main_data[0].storename !== null){ this.setState({StoreName : main_data[0].storename}) }
    if(main_data[0].status !== null){ this.setState({status : main_data[0].status}) }
    if(main_data[0].store_location !== null){ this.setState({StoreLocation : main_data[0].store_location}) }
    if(main_data[0].lat_lng !== null){ this.setState({LatLng : main_data[0].lat_lng}) }
    if(main_data[0].store_country !== null){ this.setState({CountryName : main_data[0].store_country}) }
    if(main_data[0].store_company !== null){ this.setState({Company : main_data[0].store_company}) }
    if(main_data[0].store_type !== null){ this.setState({store_type : main_data[0].store_type}) }
  }
  async handleSubmit(event) {
    ...
    fetch( server_ip+'stockCountRecords/EditStore', {
      method: 'POST',
      headers: {
  ...
  async geteditrecord(){
    let server_ip = await new_config.get_server_ip();
    return fetch( server_ip+'stockCountRecords/GetEditStoreRecord', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Connection': 'keep-alive',
						        ...
						        <div className="card-body">
						          <form autoComplete="off" id="AddStoreForm" name="registration" onSubmit={this.handleSubmit.bind(this)}>
						            ...
						            <div className="form-row">
						              ...
						              <input type="hidden" name="id" id="id" defaultValue={this.state.id} />
						              <div className="form-group  col-md-12" style={{textAlign:"center"}}>
						                <div className="row">
						                  <div className="col-md-4">
						                    <div className="but mt-5">
						                      <button type="submit" id="submit" className="btn btn-primary btn-block">Update Store
						                      </button>
						                    </div>
						                  </div>
						                </div>
