...
function abc43(){
	...
	main_table =	$('#dataTable_zero').DataTable( {
			dom: 'Bfrtip',
            buttons: [
                {
                    extend: 'excel',
	   ...
	   //      },  
			...
			'ajax': {
				'url':'/api/1.0.0/inventoryData/stockSummaryReport_zero/',
				"data": 
				function ( d ) {
					return $.extend( {}, d, {
...
var ctx = document.getElementById("barChart_zero");
...
function GraphData(){                        
     ...
     $.ajax({
         ...
         url: "/api/1.0.0/inventoryData/getStockCountData_zero",
...
function gettop20under(){
	$(document).ready(function() {
	main_table2=$('#dataTableTwoo_zero').DataTable( {
			dom: 'Bfrtip',
            buttons: [
                {
                    extend: 'excel',
			...
			'ajax': {
				'url':'/api/1.0.0/inventoryData/gettop20under_zero/',
				"data": 
				function ( d ) {
					return $.extend( {}, d, {
						"my_date": $('#summarydate').val(),
...
function gettop20over(){
	$(document).ready(function() {
	main_table3=$('#dataTableThree_zero').DataTable( {
			dom: 'Bfrtip',
            buttons: [
                {
                    extend: 'excel',
			...
			'ajax': {
				'url':'/api/1.0.0/inventoryData/gettop20over_zero/',
				"data": 
				function ( d ) {
					return $.extend( {}, d, {
						"my_date": $('#summarydate').val(),
...
$(document).ready(function() {
main_table4=$('#dataTableFour_zero').DataTable( {
			dom: 'Bfrtip',
            buttons: [
                {
                    extend: 'excel',
			...
			'serverMethod': 'post',
			'ajax': {
				'url':'/api/1.0.0/inventoryData/RealtimeDiscrepancy12_zero/',
				"data": 
				function ( d ) {
					return $.extend( {}, d, {
						"my_date": $('#summarydate').val(),
	...
	if($this.find('[name="Date"]').val()=="" || $this.find('[name="Date"]').val()==null){
 	...
 	if(error == 0){
		...
		setTimeout(function(){
        	...
        	$('.mm-datatable_zero tr th:first-child').click();

			$('.mm-dataTableTwoo_zero tr th:eq(2)').click();

			$('mm-datatable3_zero tr th:eq(2)').click();
        	
			$('.mm-dataTableFour_zero tr th:first-child').click();
		},2000);
