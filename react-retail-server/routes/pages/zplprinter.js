...
function get_zpl()
{
        ...
        $.ajax({
            ...
            url: "/api/1.0.0/stockCountRecords/getZPL",
...
function Printer(){
    ...
    $.ajax({
        ...
        url: "/api/1.0.0/stockCountRecords/getPrinterInfo",
        success: function(data)
        {
            var response_data = JSON.parse(data);
            var Printer = '';
...
$(document).ready(function(){
    ...
    $("#UID").on("input",function (){
       ...
       // {
            ...
            $.ajax({
                type:'POST',
                url:"/api/1.0.0/stockCountRecords/ZplDataProductMaster",
                data:{
                    uid:send_uui
                },
...
{}
    ...
    if($this.find('[name="Retail_Product_SupplierItemNum"]').val()=="" ){
        error=1;
        $this.find('.error_Retail_Product_SupplierItemNum').html('Please Enter Retail Product SupplierItemNum !')               
    }else{
        $this.find('.error_Retail_Product_SupplierItemNum').html(' ')
    }
    ...
    // if($this.find('[name="Printer"]').val()=="" || $this.find('[name="Printer"]').val() ==null){
    //     error=1;
    //     $this.find('.error_Printer').html('Please Select Printer !')               
    // }else{
    //     $this.find('.error_Printer').html(' ')
    // }
    ...
    if(error == 0){
       ...
       // $("#printButton").attr('disabled','disabled');
        $.ajax({
            type:'POST',
            url: "/api/1.0.0/stockCountRecords/AddPrinterForm",
    ...
    function send_vizx(
      ...
      var  Department=Department;
       var style=style;
        ...
        $.ajax({
            type:'POST',
            url: "/api/1.0.0/stockCountRecords/ConfirmApiRequestPrinter",
            data:{
                epc : epc,
                zpl : my_zpl_name,
                store_id : store_id,
