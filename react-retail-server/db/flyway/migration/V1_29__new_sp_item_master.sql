DROP PROCEDURE IF EXISTS `product_item_master_items_insert`;
DELIMITER $$
CREATE PROCEDURE `product_item_master_items_insert`(
   ...
   IN `var_style` VARCHAR(255),
   IN `var_country` VARCHAR(255),
   IN `var_supplier_no` VARCHAR(255),
   IN `var_po_supplier_no` VARCHAR(255)
...
BEGIN
        ...
        `style`= var_style,
        `country`= var_country,
        `supplier_no`= var_supplier_no,
        `po_supplier_no`= var_po_supplier_no;
        ...
        `country`= var_country,
        `supplier_no`= var_supplier_no,
        `po_supplier_no`= var_po_supplier_no where `skucode`= var_sku ;
...
UPDATE `product_item_master` SET item_code= skucode;